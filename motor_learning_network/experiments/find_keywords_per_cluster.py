...
TD_IDF_SAVING_PATH = KEYWORDS_LEVEL_DATA_PATH / f"until_{YEAR}_wordcloud_internal_scale_test_10"
...
# ── Render ────────────────────────────────────────────────────────────────
    ...
    # 1 inch = 72 points in Matplotlib
    fig_width_pts = 18 * 72 
    xlim = ax.get_xlim()
    data_range_x = xlim[1] - xlim[0]
    pts_per_data_unit = fig_width_pts / data_range_x if data_range_x > 0 else 1

    # --- THE SINGLE CONTROL PARAMETER ---
    # The top-scoring word in EVERY cluster will be exactly this size (in points).
    # All other words in that cluster will scale down relative to it.
    TARGET_MAX_FONT_PTS = 12 
    ...
    for cid in drawable:
        ...
        # --- Calculate Actual Spatial Bounds ---
        mask = (memberships == cid)
        cluster_x = all_x[mask]
        cluster_y = all_y[mask]

        min_x, max_x = cluster_x.min(), cluster_x.max()
        min_y, max_y = cluster_y.min(), cluster_y.max()

        span_x = max_x - min_x
        span_y = max_y - min_y

        # Handle rare edge cases (e.g., cluster is a single point)
        if span_x == 0: span_x = 1.0; min_x -= 0.5; max_x += 0.5
        if span_y == 0: span_y = 1.0; min_y -= 0.5; max_y += 0.5

        # Add 5% padding so text doesn't ride exactly on the outermost nodes
        pad_x = span_x * 0.05
        pad_y = span_y * 0.05
        min_x -= pad_x; max_x += pad_x; span_x += 2 * pad_x
        min_y -= pad_y; max_y += pad_y; span_y += 2 * pad_y

        # --- Dynamic Canvas Sizing to Prevent Squishing ---
        base_res = 1200
        if span_x > span_y:
            wc_width = base_res
            wc_height = max(100, int(base_res * (span_y / span_x)))
        else:
            wc_height = base_res
            wc_width = max(100, int(base_res * (span_x / span_y)))

        # --- INTERNAL NORMALIZATION (Local Sizing) ---
        # 1. Calculate how data space scales to canvas pixels
        pt_scale_factor = (span_x * pts_per_data_unit) / wc_width
        
        # 2. Translate our universal target point size into PIL pixels for this specific canvas.
        # Because PIL normalizes internally, passing this as max_font_size ensures 
        # the top word in the cluster will always hit exactly TARGET_MAX_FONT_PTS when rendered.
        target_pixels_max = max(10, int(TARGET_MAX_FONT_PTS / pt_scale_factor))

        # Initialize WordCloud matching the physical aspect ratio
        wc = WordCloud(
            width=wc_width,
            height=wc_height,
            ...
            max_font_size=target_pixels_max, # <--- Forces PIL's top word to this pixel size
            relative_scaling=1.0,            # <--- Shrinks remaining words relative to local max
        ).generate_from_frequencies(freqs)

        for item in wc.layout_:
            (word, count), f_size, (y_px, x_px), orientation, _ = item
            
            # Map PIL origin to Matplotlib coordinates
            x_data = min_x + (x_px / wc_width) * span_x
            y_data = max_y - (y_px / wc_height) * span_y
            
            # Translate pixel size back to points. 
            # The highest f_size will perfectly equal TARGET_MAX_FONT_PTS
        ...
        logger.info(
            ...
            f"spanning X({min_x:.1f} to {max_x:.1f}), Y({min_y:.1f} to {max_y:.1f})"
        )
