...
TD_IDF_SAVING_PATH = KEYWORDS_LEVEL_DATA_PATH / f"until_{YEAR}_wordcloud_test_no_min_10"
...
def _main() -> int:
    ...
    inputs = dict(
        ...
        min_cluster_size=1, # >= to this number will be included
        # wordcloud inputs
...
def save_wordcloud_figure(
    ...
    # Find the absolute highest TF-IDF score for global scaling
    global_max_tfidf = float(max(
        sc for cid in drawable for sc in cluster_freqs[cid].values()
    ))
    
    # Define how large (in points) the absolute highest scoring word should be
    BASE_MAX_FONT_PTS = 80 
    ...
    for cid in drawable:
        ...
        # --- Strict TF-IDF Font Size Mapping ---
        # 1. Calculate how data space scales to canvas pixels
        pt_scale_factor = (span_x * pts_per_data_unit) / wc_width
        
        # 2. Determine exact point size this cluster's max word deserves
        target_pts_max = BASE_MAX_FONT_PTS * (local_max / global_max_tfidf)
        
        # 3. Translate that point size into PIL pixels for the generator
        target_pixels_max = max(10, int(target_pts_max / pt_scale_factor))

        # Initialize WordCloud matching the physical aspect ratio and restricted max_font_size
        wc = WordCloud(
            width=wc_width,
            height=wc_height,
            ...
            max_font_size=target_pixels_max, # <--- Forces PIL to obey our global scale
        ...
        for item in wc.layout_:
            ...
            # Translate pixel size back to points perfectly
